# AutoCompressors - Adapting Language Models to Compress Long Contexts

This implementation is based on the paper [Adapting Language Models to Compress Long Contexts](https://arxiv.org/abs/2305.14788) (EMNLP'23), which introduces *AutoCompressors* - language models with the capability to (1) compress context information into a small set of summary vectors and (2) reason over these summary vectors which are passed to the model as soft prompts.
...
You may have to adjust some settings, like the number of GPUs, depending on your system.
...
All the fine-tuned models from the original paper can be found on Huggingface hub:

|Link|Base model|Fine-tuning seq. length|Fine-tuning data|#Summary vectors|Summary accumulation|Randomized segmenting|Softprompt stop gradient|
|-|-|-|-|-|-|-|-|
...
To load Llama-based AutoCompressor models, import LlamaAutoCompressModel:
...
If you have any questions related to the code or the paper, you can open an issue.
Please try to specify the problem with detail so we can help you quickly!

Original paper: [Adapting Language Models to Compress Long Contexts](https://arxiv.org/abs/2305.14788) by Chevalier et al.
