...
def parse_log_file(file_path):
    """
    Parses the QA_val.log file to extract questions and adversarial answers from the new format.
    The log file contains 'prompt text' with one or more questions and '[Final Answer]' with corresponding answers.
    """
    log_data = []
    current_entry = {}
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line.startswith("prompt text:"):
                # Extract prompt text and remove leading/trailing whitespace
                prompt_text = line.replace("prompt text:", "").strip()
                # Remove trailing single quote if it exists
                if prompt_text.endswith("'"):
                    prompt_text = prompt_text[:-1]
                # Split prompt text into individual questions based on '?'
                questions = [q.strip() for q in prompt_text.split("?") if q.strip()]
                # print(f"Extracted questions: {questions}")
                current_entry["question"] = questions[0]+'?'
                print(f"Current entry questions: {current_entry['question']}")
            elif line.startswith("[Final Answer]:") and current_entry.get("question"):
                # Extract the adversarial answer
                adversarial_answer = line.replace("[Final Answer]:", "").strip()
                print(f"Extracted adversarial answer: {adversarial_answer}")
                # Create an entry for the first question
                log_data.append({
                    "question": current_entry["question"],
                    "adversarial_answer": adversarial_answer
                })
                # Reset current_entry for the next prompt
                current_entry = {}
...
def prepare_comparison_messages(parquet_data, log_data):
    ...
    for log_entry in log_data:
        ...
        # print(f"Processing log entry: {question} -> {adversarial_answer}")
...
if __name__ == "__main__":
    ...
    # log_file = "path/to/Comattack/open_source_code/baselines/QA/icae/result/clean.log"
    # log_file = "path/to/Comattack/open_source_code/baselines/QA/icae/result/naive_attack.log"
    # log_file = "path/to/Comattack/open_source_code/baselines/QA/icae/result/direct_injection.log"
    # log_file = "path/to/Comattack/open_source_code/baselines/QA/icae/result/escape.log"
    log_file = "path/to/Comattack/open_source_code/baselines/QA/icae/result/context_ignore.log"
    ...
    print("Preparing comparison messages for Qwen 3...")
    ...
    if comparison_messages:
        ...
        print(f"Comparison results saved to {output_directory}/answer_consistency_qwen3.json")
