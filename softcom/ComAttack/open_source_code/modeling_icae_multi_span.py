...
class TrainingArguments(transformers.TrainingArguments):
    ...
    attack_mode: str = field(
        default='improve_target',
        metadata={"help": "The attack mode for ICAE recommendation; can be 'improve_target' or 'degrade_best'"}
    )
...
class ICAE(torch.nn.Module):
    def __init__(self, model_args, training_args, lora_config):
        ...
        if self.model_name == 'mistralai/Mistral-7B-Instruct-v0.2':
            # Use model_name_or_path or specify your model path
            self.icae = AutoModelForCausalLM.from_pretrained(model_args.model_name_or_path, torch_dtype=torch.float16 if training_args.bf16 is False else torch.bfloat16, use_flash_attention_2=True,resume_download=True)
        ...
        if self.model_name == 'mistralai/Mistral-7B-Instruct-v0.2':
            if self.training:    # indepedent model for gradient checkpointing
                # Use model_name_or_path or specify your model path
                self.decoder = AutoModelForCausalLM.from_pretrained(model_args.model_name_or_path, torch_dtype=torch.float16 if training_args.bf16 is False else torch.bfloat16,  use_flash_attention_2=True,resume_download=True)
        ...
        if self.model_name == 'mistralai/Mistral-7B-Instruct-v0.2':
            self.tokenizer = AutoTokenizer.from_pretrained(model_args.model_name_or_path, use_fast=False)
    ...
    def _compress(
        ...
        # for inference; compress a fixed length of input into memory slots
        assert (input_ids is not None or inputs_embeds is not None), "Either input_ids or inputs_embeds must be provided."

        if inputs_embeds is not None:
            total_length = inputs_embeds.size(1)
            batch_size = inputs_embeds.size(0)
            inputs_embeds = inputs_embeds.to(dtype=self.icae.dtype)
        else:
            batch_size = input_ids.size(0)
            total_length = input_ids.size(1)
        ...
        for segment_idx in range(num_segments):
            ...
            if inputs_embeds is not None:
                segment_input_embedding = inputs_embeds[:, start_idx:end_idx]
                segment_input_embedding = torch.cat([segment_input_embedding, self.memory_token_embed.weight[:self.mem_size].unsqueeze(0)], dim=1)
            else:
                segment_input_ids = input_ids[:, start_idx:end_idx]
                segment_input_ids = torch.cat([segment_input_ids, self.append_sequence], dim=1)
                mem_flag = segment_input_ids >= self.vocab_size
            ...
            # compress
            segment_input_embedding = segment_input_embedding.to(self.icae.dtype)
            segment_compress_outputs = self.icae(inputs_embeds=segment_input_embedding, output_hidden_states=True)
            segment_compress_outputs = segment_compress_outputs.hidden_states[-1]

            mem_flag = torch.zeros(segment_compress_outputs.shape[1], dtype=torch.bool, device=segment_compress_outputs.device)
            mem_flag[-self.mem_size:] = True  # use the appended memory tokens

            compress_outputs[segment_idx*self.mem_size: self.mem_size*(segment_idx+1)] = segment_compress_outputs[:, mem_flag, :].squeeze(0)
    ...
    def get_input_embeddings(self):
        ...
        return self.icae.get_base_model().model.embed_tokens
